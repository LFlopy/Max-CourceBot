...
async def _create_tables():
    async with pool.acquire() as conn:
        await conn.execute("""
            ...
            -- рассылки (кто получил)
            CREATE TABLE IF NOT EXISTS broadcast_logs (
                id              SERIAL PRIMARY KEY,
                sender_user_id  BIGINT NOT NULL,          -- админ, который отправил
                group_type      VARCHAR(50) NOT NULL,     -- тип группы (all/paid/no_sub/tariff/excluded)
                target_tariff_id INTEGER,                 -- id тарифа для bc_tariff и bc_all_except
                broadcast_at    TIMESTAMP DEFAULT NOW(),
                recipient_user_id BIGINT NOT NULL,        -- получатель рассылки
                success         BOOLEAN DEFAULT TRUE       -- true если доставлено, false если ошибка
            );
        """)
...
# ── Рассылки: учёт и подсчёт ──────────────────────────────────

async def create_broadcast_log(
    sender_user_id: int,
    group_type: str,
    target_tariff_id: int | None = None,
    recipient_user_id: int | None = None,
    success: bool = True,
) -> dict:
    """Записывает лог отправки рассылки конкретному пользователю."""
    async with pool.acquire() as conn:
        row = await conn.fetchrow("""
            INSERT INTO broadcast_logs
                (sender_user_id, group_type, target_tariff_id, recipient_user_id, success)
...
async def count_broadcast_recipients(
    ...
    """
    Подсчитывает кол-во пользователей, которым пришла рассылка.

    Параметры:
        sender_user_id  - ID админа, отправившего рассылку
        group_type      - тип группы (all/paid/no_sub/tariff/excluded), None = все
        target_tariff_id - ID тарифа для фильтрации bc_tariff и excluded
        success_only    - считать только успешные доставки

    Возвращает: int — количество доставленных рассылкок
    """
    conditions = ["sender_user_id = $1"]
    params: list = [sender_user_id]
    idx = 2

    if group_type:
        conditions.append(f"group_type = ${idx}")
        params.append(group_type)
        idx += 1

    if target_tariff_id is not None:
        conditions.append(f"target_tariff_id = ${idx}")
        params.append(target_tariff_id)
        idx += 1
    ...
    async with pool.acquire() as conn:
        sql = f"SELECT COUNT(*) FROM broadcast_logs WHERE {where_clause}"
...
async def get_broadcast_stats(
    ...
    """
    Возвращает статистику рассылки: успешные и неудачные доставки.

    Параметры те же, что у count_broadcast_recipients.
    Возвращает: {total, success, failed}
    """
    conditions = ["sender_user_id = $1"]
    params: list = [sender_user_id]
    idx = 2
    ...
    async with pool.acquire() as conn:
        total_row = await conn.fetchrow(
            f"SELECT COUNT(*) AS total FROM broadcast_logs WHERE {where_clause}",
            *params,
        )
        success_row = await conn.fetchrow(
            f"SELECT COUNT(*) AS cnt FROM broadcast_logs WHERE {where_clause} AND success = TRUE",
            *params,
        )
        failed_row = await conn.fetchrow(
            f"SELECT COUNT(*) AS cnt FROM broadcast_logs WHERE {where_clause} AND success = FALSE",
            *params,
        )

        total = int(total_row["total"]) if total_row else 0
        success = int(success_row["cnt"]) if success_row else 0
...
async def list_broadcast_recipients(
    ...
    """
    Возвращает список пользователей, которым пришла рассылка.

    Параметры те же, что у count_broadcast_recipients.
    Возвращает: [{user_id, delivered_at, success}, ...]
    """
    conditions = ["sender_user_id = $1"]
    params: list = [sender_user_id]
    idx = 2
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(f"""
            SELECT recipient_user_id AS user_id, broadcast_at AS delivered_at, success
            FROM broadcast_logs
            WHERE {where_clause}
