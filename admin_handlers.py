...
async def handle_admin_callback(bot: MaxBot, update: dict) -> bool:
    ...
    # ── Рассылка всем кроме выбранных тарифов ───────────────
    elif payload == "adm:bc_all_except":
        tariffs = await db.list_tariffs()
        excluded_ids = set(user_states.get(user_id, {}).get("excluded_tariff_ids", []))
        set_state(user_id, "adm_bc_excluded_pick", excluded_tariff_ids=excluded_ids)
        await reply(
            "Выберите тарифы, ПОДПИСЧИКАМ КОТОРЫХ не будет отправлена рассылка:\n"
            "(Все остальные получат рассылку)",
            keyboard=akb.admin_broadcast_excluded_tariff_list(tariffs, excluded_ids),
        )
    ...
    # ── Выбор тарифов для исключения из рассылки ────────────
    elif payload.startswith("adm:bc_excluded_toggle:"):
        tid = int(payload.split(":")[2])
        state_data = user_states.get(user_id, {})
        excluded_ids = set(state_data.get("excluded_tariff_ids", []))
        if tid in excluded_ids:
            excluded_ids.discard(tid)
        else:
            excluded_ids.add(tid)
        state_data["excluded_tariff_ids"] = list(excluded_ids)
        tariffs = await db.list_tariffs()
        await reply(
            "Выберите тарифы, ПОДПИСЧИКАМ КОТОРЫХ не будет отправлена рассылка:",
            keyboard=akb.admin_broadcast_excluded_tariff_list(tariffs, excluded_ids),
        ...
        if not excluded_ids:
            await reply(
                "⚠️ Нужно выбрать хотя бы один тариф для исключения",
                keyboard=akb.admin_broadcast_excluded_tariff_list(
                    await db.list_tariffs(), set()
                ),
            )
            return True
        # Сохраняем данные и переходим в режим ввода текста с группой "excluded"
        set_state(user_id, "adm_bc_text_input",
                  bc_group="excluded",
                  bc_excluded_ids=list(excluded_ids),
                  bc_tariff_id=None,
                  bc_text="",
                  bc_media=[])
        excluded_names = ", ".join(
            [t['name'] for t in (await db.list_tariffs()) if t['id'] in excluded_ids]
        )
        await reply(
            "Группа: **Всем кроме выбранных тарифов**\n\n"
            f"Исключённые тарифы: {excluded_names}\n\n"
            "Отправьте текст рассылки (можно прикрепить изображение или файл):",
            keyboard=akb.admin_broadcast_cancel(),
        )

    # ── Рассылка: отправляем без кнопок (bc_btn_none) ─────
        ...
        bc_excluded_ids = sd.get("bc_excluded_ids", [])  # исключаемые тарифы
        clear_state(user_id)
        if bc_group == "all":
        ...
        elif bc_group == "excluded":  # всем кроме выбранных тарифов
            user_ids = await db.get_users_without_tariffs(bc_excluded_ids)
        else:
            user_ids = []
        sent = 0
        for uid in user_ids:
