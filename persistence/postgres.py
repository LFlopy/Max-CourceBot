...
TARIFF_UPDATE_FIELDS = {
    "name",
    "price",
    "old_price",
    "renewal_price",
    "active_renewal_price",
    "duration_days",
    "duration_minutes",
    "duration_text",
    "is_free",
...
async def update_tariff(tariff_id: int, **fields) -> dict | None:
    ...
    unknown_fields = set(fields) - TARIFF_UPDATE_FIELDS
    if unknown_fields:
        raise ValueError(f"Unknown tariff fields: {', '.join(sorted(unknown_fields))}")
    sets = []
    vals = []
...
async def count_all_user_ids() -> int:
    """Количество всех пользователей (без заблокированных)."""
    async with pool.acquire() as conn:
        return await conn.fetchval("SELECT COUNT(*) FROM users WHERE is_banned = FALSE")
...
async def count_no_sub_user_ids() -> int:
    """Количество пользователей без единой покупки."""
    async with pool.acquire() as conn:
        return await conn.fetchval("""
...
async def count_no_paid_sub_user_ids() -> int:
    """Количество пользователей без активных платных подписок."""
    async with pool.acquire() as conn:
        return await conn.fetchval("""
...
async def count_pending_user_ids() -> int:
    """Количество пользователей с pending-покупками (начали, но не оплатили)."""
    async with pool.acquire() as conn:
        return await conn.fetchval("""
            SELECT COUNT(DISTINCT u.user_id) FROM users u
            JOIN purchases p ON p.user_id = u.user_id
...
async def count_tariff_user_ids(tariff_id: int) -> int:
    """Количество пользователей с активной подпиской на конкретный тариф."""
    async with pool.acquire() as conn:
        return await conn.fetchval("""
            SELECT COUNT(DISTINCT u.user_id) FROM users u
            JOIN purchases p ON p.user_id = u.user_id
...
async def update_promo(promo_id: int, **fields) -> dict | None:
    ...
    unknown_fields = set(fields) - PROMO_UPDATE_FIELDS
    if unknown_fields:
        raise ValueError(f"Unknown promo fields: {', '.join(sorted(unknown_fields))}")
    sets, vals = [], []
    for i, (k, v) in enumerate(fields.items(), start=2):
        sets.append(f"{k} = ${i}")
        vals.append(v)
...
async def confirm_purchase(purchase_id: int, expires_at=None) -> dict | None:
...
async def update_warmup_message(message_id: int, **fields) -> dict | None:
    ...
    unknown_fields = set(fields) - WARMUP_MESSAGE_UPDATE_FIELDS
    if unknown_fields:
        raise ValueError(f"Unknown warmup message fields: {', '.join(sorted(unknown_fields))}")
    sets = []
    vals = []
    for i, (k, v) in enumerate(fields.items(), start=2):
        sets.append(f"{k} = ${i}")
